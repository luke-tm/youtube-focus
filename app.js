...
document.addEventListener(‘DOMContentLoaded’, init);function onAuthChange(authed) {
  state.authed = authed;
  if (authed) loadInitialData(); else render();
}
...
document.addEventListener('DOMContentLoaded', init);    var profile = await YT_API.getMyChannel();
    state.userProfile = profile;
  } catch (e) {
